...
def main():
    ...
    try:
        ...
        executer(cur, "CREATE DATABASE IF NOT EXISTS NYC_TAXI_DB", "Database NYC_TAXI_DB")
        ...
        for row in rows:
            if "SERVICE_ACCOUNT" in str(row[0]).upper():
                ...
                print(f"     {row[2]}")
                print(f"     → Rôle requis : Storage Object Admin\n")
        ...
        if blob.exists():
            print(f"  ✓ Déjà présent dans GCS — rien à faire")
        else:
